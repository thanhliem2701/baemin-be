...
export class AuthService {
    ...
    async login(username: string, password: string) {
        ...
        if (!isMatch) return ErrorMessages.PASSWORD_INCORRECT;
    ...
    async verify(token: string, sercretCode: string) {
        ...
        try {
            if (sercretCode == ConstantCodes.TOKEN_CODE) {
                ...
                return payload;
            }else return ErrorMessages.TOKEN_VERIFICATION_FAILED;
        } catch (error) {
            switch (error.name) {
                ...
                case ConstantCodes.TOKEN_NOT_ACTIVE:
                    return ErrorMessages.TOKEN_NOT_ACTIVE;
                default:
                    return ErrorMessages.TOKEN_VERIFICATION_FAILED;
            }
        }
    }
    ...
    async generateToken(username: string, status: string) {
        ...
        const token = await this.jwtService.signAsync({ data: { username, status } },
            { algorithm: 'HS256', expiresIn: '1d', secret: this.configService.get<string>("SECRET_KEY") });
        ...
        const refresh_token = await this.jwtService.signAsync({ data: { username, status: ConstantCodes.TOKEN_EXPIRED } },
            { algorithm: 'HS256', expiresIn: '7d', secret: this.configService.get<string>("REFRESH_TOKEN_SECRET_KEY") });
